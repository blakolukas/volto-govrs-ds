...
const StepVertical = ({
  ...
  const updateLabelSlot = useCallback(() => {
    ...
    const labels = containerRef.current.querySelectorAll(
      '.v-step-label.right, .v-step-label.left',
    );
  ...
  const inlineStyle = {
    ...
    ...(measuredConnectorGap
      ? { '--v-connector-gap': `${measuredConnectorGap}px` }
      : {}),
  };
  ...
  return (
    <div
      ref={containerRef}
      style={inlineStyle}
      className={`br-step-vertical label-${labelPosition} ${variantClass}`}
      role="list"
    >
      {steps.map((step, idx) => {
        ...
        if (isPlainVariant) {
          const circleCls =
            `v-step-circle ${isActive ? 'active' : ''} plain`.trim();
          let circleNode = null;
              ...
              <React.Fragment key={idx}>
                <div className={itemClass} role="listitem">
                  {labelPosition === 'left' && step.label && (
                    <div className={`v-step-label left`}>{step.label}</div>
                  )}
                  <Tooltip
                    position={tipPos}
                    state={step.state}
                  ...
                  {labelPosition === 'right' && step.label && (
                    <div className={`v-step-label right`}>{step.label}</div>
                  )}
                </div>
              </React.Fragment>
            );
          ...
          <React.Fragment key={idx}>
            {isDottedVariant && step.state ? (
              <div className={itemClass} role="listitem">
                ...
                {step.label && labelPosition === 'above' && (
                  <div className={`v-step-label above`}>{step.label}</div>
                )}
                {(() => {
                  const baseCls =
                    `v-step-status v-step-status--${step.state} v-step-status--dotted ${isActive ? 'active' : ''} ${isClickable ? 'clickable' : ''}`.trim();
                  let statusNode = null;
                  if (step.href) {
                    statusNode = (
                      <a
              ...
              <div className={itemClass} role="listitem">
                ...
                {(() => {
                  const circleCls =
                    `v-step-circle ${isActive ? 'active' : ''} ${isDottedVariant ? 'dotted' : ''}`.trim();
                  let circleNode = null;
                  ...
                  if (step.href) {
                    circleNode = (
                      ...
                      >
                        {!isDottedVariant &&
                          (step.icon ? (
                            <span className="v-step-icon">{step.icon}</span>
                          ) : (
                            <span className="v-step-number">{number}</span>
                          ))}
                        {!isDottedVariant && step.state && (
                          <span
                            className={`v-step-status v-step-status--${step.state}`}
                          >
                ...
                {!isDottedVariant &&
                  step.label &&
                  labelPosition === 'below' && (
                    <div className={`v-step-label below`}>{step.label}</div>
                  )}
              </div>
            )}
            ...
            {idx < steps.length - 1 &&
              ...
              (() => {
                ...
                const cls =
                  `v-connector ${topLarge ? 'adj-top-large' : ''} ${bottomLarge ? 'adj-bottom-large' : ''}`.trim();
