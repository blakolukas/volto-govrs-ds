...
const Edit = (props) => {
  ...
  React.useEffect(() => {
    properties.blocks_layout.items.map((item) => {
      const panelBlock = properties.blocks[item];

      // Ensure panel has @type set
      if (panelBlock && !panelBlock['@type']) {
        onChangeBlock(block, {
          ...data,
          data: {
            ...properties,
      ...
      // Ensure panel has blocks initialized
      if (isEmpty(panelBlock?.blocks)) {
